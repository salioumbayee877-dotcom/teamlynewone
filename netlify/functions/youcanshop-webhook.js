...
exports.handler = async (event) => {
  ...
  try {
    ...
    const city      = extracted?.city || (isJunkCity(addrObj.city) ? "" : addrObj.city) || "";
    const citySearch = city || address || addrObj.address || addrObj.address_1 || "";
    ...
    try {
      await sendPush({
        orgId,
        roles: ["admin", "closer"],
        title: "🛒 Nouvelle commande YouCan",
