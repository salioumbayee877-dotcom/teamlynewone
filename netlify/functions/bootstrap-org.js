// ═══════════════════════════════════════════════════════════════
// TEAMLY — bootstrap-org
// Crea (o devuelve) la organización + profile admin para el usuario
// autenticado. Usa SERVICE_KEY para bypassar RLS, igual que google-onboard.
//
// Reemplaza el INSERT client-side que en handleRegister y en el fallback
// de login fallaba con HTTP 403 / 42501 "new row violates row-level
// security policy for table organizations".
//
// POST /.netlify/functions/bootstrap-org
//   Headers: Authorization: Bearer <user_jwt>
//   Body: { boutique: "Ma Boutique", phone: "221...", nom?: "Ibou" }
//   Response: { ok: true, orgId, role: "admin", nom, alreadyExists?: true }
// ═══════════════════════════════════════════════════════════════
...
exports.handler = async (event) => {
  ...
  const boutique = (body.boutique || "Ma Boutique").trim().slice(0, 120);
  const phone    = (body.phone || "").trim().slice(0, 30);
  const nom      = (body.nom || user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split("@")[0] || "Admin").slice(0, 60);

  // If user already has a profile/org, return it (idempotent)
  try {
    const pr = await fetch(`${SB_URL}/rest/v1/profiles?id=eq.${user.id}&select=org_id,role,nom&limit=1`, { headers: sbHeaders });
    const rows = await pr.json().catch(() => []);
    if (Array.isArray(rows) && rows[0]?.org_id) {
      return { statusCode: 200, headers, body: JSON.stringify({ ok: true, orgId: rows[0].org_id, role: rows[0].role || "admin", nom: rows[0].nom || nom, alreadyExists: true }) };
    }
  } catch (e) {
  ...
  try {
    const orgRes = await fetch(`${SB_URL}/rest/v1/organizations`, {
      method: "POST",
      headers: sbHeaders,
      // Plan "pro" offert au lancement (paiement en ligne pas encore prêt) :
      // les nouveaux comptes ont accès à toutes les fonctionnalités Pro.
      body: JSON.stringify({ id: orgId, name: boutique, whatsapp: phone, plan: "pro" }),
    });
    if (!orgRes.ok) {
