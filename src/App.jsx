...
import {
  ...
  Banknote, Pin as Pushpin, Percent, Hash, Download, FlaskConical, Share2,
} from "lucide-react";
...
function AppInner() {
  ...
  useEffect(()=>{
    let pendingRef = ""; try{ pendingRef = localStorage.getItem("teamly_pending_ref")||""; }catch(e){}
    const ref = (pendingRef || "").toUpperCase().trim();
    if (ref && !promoCode.trim() && !(appliedPromo && !appliedPromo._referral)) {
      setAppliedPromo({ code: ref, discount_pct: REFERRAL_DISCOUNT_PCT, _referral: true });
      setPromoApplied(true);
    } else if (!ref && appliedPromo?._referral && !promoCode.trim()) {
      setAppliedPromo(null); setPromoApplied(false);
    }
  }, [promoCode, authStep]);
  ...
  // Paiement d'abonnement via Intech (mobile money)
  const [intechPanel,    setIntechPanel]    = useState(null);   // planKey dont le panneau est ouvert
  ...
  const [myReferralCode, setMyReferralCode] = useState("");      // código del parrain (esta org)
  ...
  const startWavePayment = async (amount=13000, planKey="basic") => {
    ...
    try {
      ...
      if(promoApplied && appliedPromo?.code && !appliedPromo._referral){
      ...
      // Guardar monto pagado para acreditar la comisión de parrainage tras el pago
      try{ localStorage.setItem("teamly_pending_amount", String(amount)); }catch(e){}
  ...
  // Paiement d'abonnement via Intech (mobile money). Le PRIX est calculé côté
  // serveur (intech-plan-checkout) — on n'envoie jamais le montant. Le plan est
  ...
  const startIntechPlanPayment = async (planKey, codeService, phone) => {
    ...
    try {
      ...
      const res = await fetch("/.netlify/functions/intech-plan-checkout",{
        method:"POST",
        headers:{"Content-Type":"application/json","Authorization":`Bearer ${_authToken||SB_KEY}`},
        // promoCode volontairement null : le descuento promo passe par Wave (v1).
      ...
      const poll = async () => {
        try {
          const r = await fetch(`/.netlify/functions/intech-status?externalTransactionId=${encodeURIComponent(extId)}`,{ headers:{"Authorization":`Bearer ${_authToken||SB_KEY}`} });
          const row = await r.json();
          if(row?.status==="SUCCESS"){
            setIntechPayState("SUCCESS"); setIsPro(true); setTrialDaysLeft(31);
            setSettings(s=>({...s, plan:planKey}));
            addToast("Paiement confirmé — plan activé 🎉","✅","#1A5C38");
            setTimeout(()=>{ setShowPlanModal(false); setIntechPanel(null); setIntechPayState("idle"); }, 1500);
            return;
          }
  ...
  useEffect(()=>{
    ...
    if(params.get("payment")==="success" && params.get("org")===orgId && orgId) {
      ...
      try{ pendingAmount = localStorage.getItem("teamly_pending_amount"); }catch(e){}
          ...
          if(pending) {
            ...
            try {
              const br = await fetchWithTimeout("/.netlify/functions/bootstrap-org", {
                ...
                body: JSON.stringify({ boutique: boutique || "Ma Boutique", phone: phone || "", nom: nom || "Admin", refCode: (typeof localStorage!=="undefined"?(localStorage.getItem("teamly_pending_ref")||""):"") }),
              }, 20000);
  ...
  const loadReferrals = async () => {
    try {
      const rows = await sbFetch(`referrals?referrer_org_id=eq.${orgId}&select=*&order=created_at.desc`);
  ...
  const PLANS = [
    ...
    },
    {
      ...
      features:[
        ...
        "Confirmation WhatsApp automatique",
        "Suivi de la commande par le client",
      ...
      .then(async(data)=>{
        ...
        try {
          const r = await fetchWithTimeout("/.netlify/functions/bootstrap-org", {
            ...
            body: JSON.stringify({ boutique: authForm.boutique || "Ma Boutique", phone: authForm.phone || "", nom: derivedNom, refCode: (typeof localStorage!=="undefined"?(localStorage.getItem("teamly_pending_ref")||""):"") }),
          }, 20000);
  ...
  const handleGoogleOnboardSubmit = async () => {
    ...
    try {
      const r = await fetchWithTimeout("/.netlify/functions/google-onboard", {
        ...
        body: JSON.stringify({ boutique: authForm.boutique.trim(), phone: authForm.phone.trim(), nom, refCode: (typeof localStorage!=="undefined"?(localStorage.getItem("teamly_pending_ref")||""):"") }),
      }, 15000);
        ...
        <div style={{width:"100%",maxWidth:360}}>
                ...
                try {
                  ...
                  if(profiles&&profiles.length>0){
                    ...
                    const r = await fetchWithTimeout("/.netlify/functions/bootstrap-org", {
                      method: "POST",
                      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${tok}` },
                      body: JSON.stringify({ boutique: "Ma Boutique", phone: "", nom: authForm.email.split("@")[0], refCode: (typeof localStorage!=="undefined"?(localStorage.getItem("teamly_pending_ref")||""):"") }),
                    }, 20000);
                    const j = await r.json().catch(()=>({}));
                    if (!r.ok || !j.ok) throw new Error(j?.error || `HTTP ${r.status}`);
          ...
          {authMode==="register"&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              {/* Code de parrainage (optionnel) — remise sur le 1er paiement */}
              <div>
                <div style={{fontSize:11,color:"rgba(255,255,255,0.6)",marginBottom:4,fontFamily:"sans-serif",display:"flex",alignItems:"center",gap:5}}><Gift size={12}/> Code de parrainage (optionnel)</div>
                <input
                  type="text"
                  name="refCode"
                  ...
                  onChange={e=>{
                    ...
                    setAuthForm(p=>({...p,refCode:v}));
                    try{ if(v) localStorage.setItem("teamly_pending_ref",v); else localStorage.removeItem("teamly_pending_ref"); }catch(err){}
                  }}
                  placeholder="Ex: TMLY-3F7K"
                  style={{width:"100%",background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:10,padding:"11px 14px",fontSize:13,color:G.white,outline:"none",boxSizing:"border-box",fontFamily:"sans-serif",textTransform:"uppercase",letterSpacing:0.5}}/>
                {authForm.refCode && <div style={{fontSize:10,color:"#86EFAC",marginTop:4,fontFamily:"sans-serif"}}>🎁 −{REFERRAL_DISCOUNT_PCT}% sur ton 1er abonnement</div>}
              </div>
              {authError&&<div style={{fontSize:11,color:"#FCA5A5",fontFamily:"sans-serif"}}>{authError}</div>}
      ...
      {sessionExpired&&(
        <div style={{position:"fixed",inset:0,background:"rgba(15,25,35,0.96)",zIndex:5500,display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"sans-serif"}}>
          <div style={{maxWidth:380,width:"100%",textAlign:"center"}}>
            <div style={{marginBottom:14,display:"flex",justifyContent:"center"}}><LogOut size={54} color="#fff"/></div>
          ...
          try {
            ...
            try {
              const br = await fetchWithTimeout("/.netlify/functions/bootstrap-org", {
                method: "POST",
                headers: { "Content-Type": "application/json", "Authorization": `Bearer ${tok}` },
                body: JSON.stringify({ boutique, phone, nom, refCode: (typeof localStorage!=="undefined"?(localStorage.getItem("teamly_pending_ref")||""):"") }),
              }, 20000);
              const bj = await br.json().catch(()=>({}));
              if (br.ok && bj.ok && bj.orgId) newOrgId = bj.orgId;
        ...
        <div className="tl-scroll-dark" style={{flex:1,overflowY:"auto",padding:"10px 0"}}>
          ...
          {tabDef[role].map(t=>{
            ...
            <button key={t.k} onClick={()=>{setTab(t.k);setSidebarOpen(false);}}
              ...
              {t.k==="livraisons"&&(()=>{const cnt=filteredOrders.filter(o=>o.livreur_id===currentUser.id&&LIV_ACTIVE.has(o.status)).length;return cnt>0?<span style={{background:"#0284C7",color:G.white,borderRadius:"50%",width:18,height:18,fontSize:10,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center"}}>{cnt}</span>:null;})()}
            </button>
            );
          })}
      ...
      {showNotifPanel&&(
        <div style={{position:"fixed",top:56,right:12,width:310,background:"#fff",borderRadius:16,boxShadow:"0 8px 32px rgba(0,0,0,0.18)",zIndex:9999,overflow:"hidden",border:"1px solid #E5E7EB"}}>
          <div style={{padding:"12px 16px",borderBottom:"1px solid #F3F4F6",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontWeight:700,fontSize:14,color:G.dark,display:"inline-flex",alignItems:"center",gap:5}}><Bell size={14}/> Notifications</span>
          ...
          <div style={{maxHeight:360,overflowY:"auto"}}>
            {dbNotifs.length===0?(
              ...
              <div key={n.id} style={{padding:"12px 16px",borderBottom:"1px solid #F9FAFB",display:"flex",gap:10,alignItems:"flex-start"}}>
                <span style={{flexShrink:0,display:"inline-flex"}}>{n.type==="delivered"?<Check size={22} color={G.green}/>:n.type==="rejected"?<X size={22} color={G.red}/>:n.type==="nouveau_colis"?<Bell size={22}/>:n.type==="status_update"?<Package size={22}/>:<Bell size={22}/>}</span>
                <div style={{flex:1}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              ...
              {leads.length===0?(
                <div style={{textAlign:"center",padding:40,color:G.gray,background:G.white,borderRadius:14}}>
                  <div style={{marginBottom:12,display:"flex",justifyContent:"center"}}><Mail size={48} color={G.gray}/></div>
                    ...
                    <div key={o.id} style={{background:G.white,borderRadius:14,boxShadow:"0 2px 8px rgba(0,0,0,0.08)",borderLeft:`4px solid ${productRecognized?"#10B981":"#F59E0B"}`,overflow:"hidden"}}>
                          ...
                          <div>
                            <div style={{display:"flex",alignItems:"center",gap:6}}>
                              ...
                              {o.created_at&&<span style={{fontSize:10,color:G.gray,background:G.grayLight,borderRadius:5,padding:"1px 6px"}}>
                                <Clock size={11} style={{display:"inline",verticalAlign:"-2px"}}/> {new Date(o.created_at).toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}
                              </span>}
                            </div>
                            {(()=>{
                              ...
                              return (
                                <div style={{marginTop:3}}>
                                  {isMulti&&<div style={{display:"inline-flex",alignItems:"center",gap:4,background:"#FEF3C7",borderRadius:6,padding:"2px 7px",fontSize:10,fontWeight:800,color:"#92400E",marginBottom:4}}>
                                    <Package size={11}/> BUNDLE · {tot} article{tot>1?"s":""}</div>}
                                  <div style={{display:"flex",flexDirection:"column",gap:2}}>
                                    {items.map((p,pi)=>(
                                      <div key={pi} style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
                                        <span style={{fontSize:12,color:G.gray,display:"inline-flex",alignItems:"center",gap:4}}><Package size={12}/> {p.name}</span>
                                        ...
                                        {pi===0&&(isMatched||catalogMatch
                                          ?<span style={{background:"#D1FAE5",color:"#065F46",borderRadius:5,padding:"1px 6px",fontSize:10,fontWeight:700,flexShrink:0,display:"inline-flex",alignItems:"center",gap:3}}><Check size={10}/> Reconnu</span>
                                          :isAutoCreated
                                            ?<span style={{background:"#EDE9FE",color:"#5B21B6",borderRadius:5,padding:"1px 6px",fontSize:10,fontWeight:700,flexShrink:0,display:"inline-flex",alignItems:"center",gap:3}}><Star size={10}/> Ajouté</span>
                                            :<span style={{background:"#FEF3C7",color:"#92400E",borderRadius:5,padding:"1px 6px",fontSize:10,fontWeight:700,flexShrink:0,display:"inline-flex",alignItems:"center",gap:3}}><AlertTriangle size={10}/> Inconnu</span>
                                        )}
                                      </div>
                                    ))}
                                  </div>
                                  <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap",alignItems:"center"}}>
                                    <span style={{fontSize:11,color:G.gray,display:"inline-flex",alignItems:"center",gap:4}}><MapPin size={11}/> {fullAddr(o)}</span>
                                    {(()=>{const f=fraisDisplay(o);return(<><span style={{background:z.color+"18",color:z.color,borderRadius:5,padding:"1px 6px",fontSize:10,fontWeight:700,display:"inline-flex",alignItems:"center",gap:3}}>{z.flag} {z.label} · {f.label}{f.warning&&<AlertTriangle size={10} color={G.gold} style={{marginLeft:2}}/>}</span>{f.warning&&<button onClick={e=>{e.stopPropagation();openFraisAndDismiss();}} title="Configurer les zones" style={{background:G.gold,color:"#fff",border:"none",borderRadius:5,padding:"2px 6px",fontSize:10,fontWeight:700,cursor:"pointer",lineHeight:1,display:"inline-flex",alignItems:"center"}}><IcoSettings size={11}/></button>}</>);})()}
                                    {z.prepaid&&<span style={{background:"#FEF3C7",color:"#92400E",borderRadius:5,padding:"1px 6px",fontSize:10,fontWeight:700,display:"inline-flex",alignItems:"center",gap:3}}><AlertTriangle size={10}/> Prépayé</span>}
                                  </div>
                                  <div style={{fontSize:11,color:G.gray,marginTop:2,display:"flex",alignItems:"center",gap:4}}><Smartphone size={11}/> {o.phone||"—"}</div>
                      ...
                      <div style={{display:"flex",gap:7}}>
                        ...
                        <button onClick={()=>{ upSt(o.id,"rechazado"); addToast(`Commande annulée ❌`,"❌",G.red); }}
                          style={{background:"#FEE2E2",color:G.red,border:"none",borderRadius:10,padding:"10px 10px",fontWeight:700,fontSize:12,cursor:"pointer",display:"inline-flex",alignItems:"center",justifyContent:"center"}}>
                          <X size={16}/>
                        </button>
...
<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><Package size={20} color={G.gray}/></div>
        ...
        {tab==="superadmin"&&String(currentUser.email||"").toLowerCase().trim()===OWNER_EMAIL&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {/* ── PARRAINAGES (gestion des commissions) ── */}
            <div style={{background:"#FFF",borderRadius:14,boxShadow:"0 2px 8px rgba(0,0,0,0.06)",overflow:"hidden"}}>
              <button onClick={async()=>{
                const nv=!saReferralOpen; setSaReferralOpen(nv);
                if(nv){
                  setSaReferralLoading(true);
                  try{
                    const res = await fetch("/.netlify/functions/super-admin?view=referrals",{ headers:{"Authorization":`Bearer ${_authToken}`} });
                    const data = await res.json();
                    if(data && Array.isArray(data.referrals)){ setSaReferrals(data.referrals); setSaReferralTotals(data.totals||{pending:0,paid:0}); }
                    else addToast("Erreur chargement parrainages","❌",G.red);
                  } catch(e){ addToast("Erreur connexion","❌",G.red); }
                  setSaReferralLoading(false);
                }
              }} style={{width:"100%",background:"linear-gradient(135deg,#1A3828,#0D1F14)",color:"#FFF",border:"none",padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <Gift size={18} color={G.gold}/>
                  <div style={{textAlign:"left"}}>
                    <div style={{fontWeight:800,fontSize:14,color:"#FFF"}}>Parrainages</div>
                    <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",marginTop:1}}>Commissions des parrains · clique pour gérer</div>
                  </div>
                </div>
                <span style={{fontSize:18,color:"rgba(255,255,255,0.7)",transform:saReferralOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▾</span>
              </button>
              ...
              {saReferralOpen && (
                <div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:10}}>
                  {/* Totaux */}
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    <div style={{background:"#FEF9E7",borderRadius:10,padding:"10px 12px",borderLeft:`4px solid ${G.gold}`}}>
                      <div style={{fontSize:10,color:"#92400E",fontWeight:700}}>À payer</div>
                      <div style={{fontSize:18,fontWeight:800,color:G.dark}}>{fmt(saReferralTotals.pending||0)} <span style={{fontSize:11,color:G.gray}}>CFA</span></div>
                    </div>
                    <div style={{background:G.greenLight,borderRadius:10,padding:"10px 12px",borderLeft:`4px solid ${G.green}`}}>
                      <div style={{fontSize:10,color:G.green,fontWeight:700}}>Déjà payé</div>
                      <div style={{fontSize:18,fontWeight:800,color:G.dark}}>{fmt(saReferralTotals.paid||0)} <span style={{fontSize:11,color:G.gray}}>CFA</span></div>
                    </div>
                  </div>

                  {saReferralLoading ? (
                    <div style={{textAlign:"center",padding:14,color:G.gray,fontSize:12}}>Chargement…</div>
                  ) : saReferrals.length>0 ? (
                    <div style={{display:"flex",flexDirection:"column",gap:6}}>
                      {saReferrals.map(r=>{
                        const stColor = {pending:G.gray,converted:G.gold,paid:G.green}[r.status]||G.gray;
                        const stLabel = {pending:"En attente",converted:"À payer",paid:"Payé"}[r.status]||r.status;
                        return (
                        <div key={r.id} style={{background:"#F9FAFB",borderRadius:10,padding:"10px 12px",borderLeft:`4px solid ${stColor}`}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                            <div style={{minWidth:0,flex:1}}>
                              <div style={{fontWeight:700,fontSize:13,color:G.dark}}>{r.referrerName}<span style={{color:G.gray,fontWeight:500}}> → {r.referredOrgName}</span></div>
                              <div style={{fontSize:10,color:G.gray,marginTop:2}}>
                                {r.referrerAdmin||r.referrerEmail||"—"}{r.referrerPhone?` · ${r.referrerPhone}`:""}
                              </div>
                              <div style={{fontSize:10,color:G.gray,marginTop:1}}>{new Date(r.created_at).toLocaleDateString("fr-FR")}{r.plan?` · ${r.plan}`:""} · code {r.code}</div>
                            </div>
                            <div style={{textAlign:"right",flexShrink:0}}>
                              <div style={{fontSize:14,fontWeight:800,color:G.dark}}>{r.commission_cfa?`${fmt(r.commission_cfa)}`:"—"}<span style={{fontSize:9,color:G.gray}}> CFA</span></div>
                              <div style={{fontSize:9,fontWeight:700,color:stColor,textTransform:"uppercase",letterSpacing:0.5,marginTop:2}}>{stLabel}</div>
                            </div>
                          </div>
                          {r.status==="converted" && (
                            <button onClick={()=>{
                              setConfirmModal({
                                msg:`Marquer payé : ${fmt(r.commission_cfa)} CFA ?`,
                                sub:`Parrain : ${r.referrerName}\nFilleul : ${r.referredOrgName}`,
                                onConfirm: async()=>{
                                  try{
                                    const res = await fetch("/.netlify/functions/super-admin?view=referrals",{
                                      method:"PATCH",
                                      headers:{"Content-Type":"application/json","Authorization":`Bearer ${_authToken}`},
                                    ...
                                    if(d.success){
                                      setSaReferrals(prev=>prev.map(x=>x.id===r.id?{...x,status:"paid"}:x));
                                      setSaReferralTotals(t=>({pending:Math.max(0,(t.pending||0)-(r.commission_cfa||0)),paid:(t.paid||0)+(r.commission_cfa||0)}));
                                      addToast("Commission marquée payée ✅","✅",G.green);
                                    } else addToast(d.error||"Erreur","❌",G.red);
                                  } catch(e){ addToast("Erreur connexion","❌",G.red); }
                                },
                              });
                            }} style={{marginTop:8,width:"100%",background:G.green,color:"#FFF",border:"none",borderRadius:8,padding:"8px 0",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                              Marquer payé
                            </button>
                          )}
                        </div>
                    ...
                    <div style={{textAlign:"center",padding:14,color:G.gray,fontSize:12,background:"#F9FAFB",borderRadius:8}}>Aucun parrainage pour l'instant</div>
                  )}
                </div>
              )}
            </div>
        ...
        {dataReady&&tab==="dashboard"&&role==="livreur"&&(()=>{
          ...
          const toPickup   = myLiv.filter(o=>o.status==="livreur_en_route"&&o.region_type!=="other");
          const toPickupOther = myLiv.filter(o=>o.region_type==="other"&&o.status==="paiement_confirme");
          const toColisOther = myLiv.filter(o=>o.region_type==="other"&&o.status==="livreur_en_route");
...
<AlertTriangle size={14} style={{display:"inline",verticalAlign:"-3px"}}/> Aucune livraison assignée à <strong>{currentUser.nom}</strong>. Demande à l'Admin de t'assigner des commandes.
            ...
            {toPickupOther.length>0&&(
              <div style={{background:"#E8F5EE",borderRadius:16,border:"2px solid #86EFAC",padding:"18px 16px 20px",boxShadow:"0 2px 12px rgba(0,0,0,0.08)",animation:"livFadeIn 220ms ease"}}>
                ...
                <button disabled={isBatch}
                  onClick={()=>batchAdvance(toPickupOther,"livreur_en_route","En route pour récupérer 🏍️")}
                  ...
                  {isBatch?"…":(toPickupOther.length>1?"Je pars récupérer les colis":"Je pars récupérer le colis")}
                </button>
              </div>
            )}

            {/* ── BATCH NOTIFICATION OTHER — Colis en main (interurbain, en route → colis_en_main) ── */}
            {toColisOther.length>0&&toPickupOther.length===0&&(
              <div style={{background:"#DBEAFE",borderRadius:16,border:"2px solid #93C5FD",padding:"18px 16px 20px",boxShadow:"0 2px 12px rgba(0,0,0,0.08)",animation:"livFadeIn 220ms ease"}}>
                ...
                <div style={{background:"rgba(255,255,255,0.75)",borderRadius:10,padding:"10px 12px",marginBottom:14}}>
                  {toColisOther.slice(0,5).map((o,i)=>{
                    const ref=o.note?.match(/#[\w-]+/)?.[0]||"";
                    return (
                  ...
                  {toColisOther.length>5&&<div style={{fontSize:11,color:"#2563EB",marginTop:5,fontWeight:600}}>+{toColisOther.length-5} autres colis…</div>}
                </div>
                <button disabled={isBatch}
                  onClick={()=>batchAdvance(toColisOther,"colis_en_main","Colis en main 📦")}
                  style={{width:"100%",background:isBatch?"#9CA3AF":"#2563EB",color:"#fff",border:"none",borderRadius:12,padding:"16px 0",fontWeight:800,fontSize:15,cursor:isBatch?"not-allowed":"pointer",transition:"background 150ms"}}>
                  {isBatch?"…":(toColisOther.length>1?"J'ai les colis en main":"J'ai le colis en main")}
                </button>
              </div>
            )}
...
<div style={{marginBottom:10,display:"flex",justifyContent:"center"}}><Bell size={44} color={G.gray}/></div>
        ...
        {dataReady&&tab==="chat"&&(()=>{
          ...
          <div style={{display:"flex",flexDirection:"column",margin:chatMargin,height:chatH,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{background:G.green,padding:"12px 16px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
              ...
              <div style={{width:42,height:42,borderRadius:"50%",background:"rgba(255,255,255,0.2)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Users size={22} color="#fff"/></div>
              <div style={{flex:1,minWidth:0}}>
            ...
            <div ref={chatScrollRef}
              ...
              {!chatLoading&&chat.length===0&&(
                <div style={{textAlign:"center",padding:40,color:"#8a9a8a",display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
                  <MessageCircle size={36} color={G.gray}/>
      ...
      <RotateCcw size={14} style={{display:"inline",verticalAlign:"-2px"}}/> Actualiser
                  </button>
                </div>
              )}
              {chat.map((msg,i)=>{
...
<Mic size={16} style={{opacity:playingMsgId===msg.id?1:0.5}}/>
                          <style>{`
      ...
      <Trash2 size={13} style={{display:"inline",verticalAlign:"-2px"}}/> Supprimer
                      </button>
                    )}
                  </div>
                );
      ...
      {false&&(()=>{
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{background:G.white,borderRadius:"20px 20px 0 0",padding:22,width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto"}}>
            <div style={{fontWeight:700,fontSize:17,color:G.green,marginBottom:4,display:"flex",alignItems:"center",gap:6}}><IcoMap size={18}/> Zones de livraison</div>
            ...
            <div style={{background:"#F0FDF4",borderRadius:14,padding:14,marginBottom:14,border:"1.5px solid #86EFAC"}}>
              <div style={{fontSize:13,fontWeight:700,color:"#166534",marginBottom:10,display:"flex",alignItems:"center",gap:5}}><Bike size={14}/> Région principale de vente</div>
              {zoneMainEdit ? (
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  <div style={{display:"flex",gap:6}}>
                  ...
                  <div style={{display:"flex",gap:8,marginTop:4}}>
                    <button onClick={saveMainRegion} style={{flex:1,background:G.green,color:"#fff",border:"none",borderRadius:10,padding:10,fontWeight:700,fontSize:13,cursor:"pointer",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:5}}><Check size={14}/> Enregistrer</button>
      ...
      {showSettings&&(
        <div onClick={()=>setShowSettings(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:200,display:"flex",alignItems:isDesktop?"center":"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:G.white,borderRadius:isDesktop?20:"20px 20px 0 0",padding:22,width:"100%",maxWidth:480,margin:"0 auto",maxHeight:"90vh",overflowY:"auto"}}>
            ...
            <div style={{marginBottom:18}}>
              <div style={{fontSize:12,fontWeight:700,color:G.gray,marginBottom:4,letterSpacing:0.5,display:"flex",alignItems:"center",gap:5}}><KeyRound size={13}/> PERMISSION CLOSER</div>
...
<Trash2 size={14} style={{display:"inline",verticalAlign:"-3px"}}/> Supprimer mon compte
            </button>
...
<Check size={14} style={{display:"inline",verticalAlign:"-2px"}}/> Enregistrer
            ...
            <div style={{marginBottom:18}}>
              ...
              {[
                {key:"nom",      Ico:User,       label:"Ton nom",          ph:"Ton nom"},
                {key:"phone",    Ico:Smartphone, label:"Téléphone",         ph:"221 77 123 45 67"},
                {key:"birthday", Ico:Cake,       label:"Date de naissance", ph:"JJ/MM/AAAA", type:"date"},
              ].map(f=>(
                <div key={f.key} style={{marginBottom:10}}>
                  <div style={{fontSize:11,color:G.gray,marginBottom:3,display:"flex",alignItems:"center",gap:4}}><f.Ico size={12}/> {f.label}</div>
...
<Trash2 size={14} style={{display:"inline",verticalAlign:"-3px"}}/> Supprimer mon compte
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:G.white,borderRadius:isDesktop?20:"20px 20px 0 0",width:"100%",maxWidth:480,padding:20,paddingBottom:32}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div>
                ...
                <div style={{fontSize:11,color:G.gray,marginTop:2,display:"flex",alignItems:"center",gap:4}}>{memberModal.role==="closer"?<><Phone size={11}/> Closer</>:<><Bike size={11}/> Livreur</>} · {memberModal.email}</div>
              </div>
                    ...
                    <div style={{padding:"12px 16px"}}>
                      ...
                      {!isCurrent && isPaidPlan && !isOwner && !PLAN_COMING_SOON(p.key) && !PLAN_LAUNCH_FREE(p.key) && (
                        <div style={{marginTop:8}}>
                          {intechPanel!==p.key ? (
                            <button onClick={()=>{ setIntechPanel(p.key); setIntechPayState("idle"); }}
                              style={{width:"100%",background:"#FFF",color:p.color,border:`1.5px solid ${p.color}`,borderRadius:10,padding:"10px 0",fontWeight:700,fontSize:12,cursor:"pointer"}}>
                              📲 Payer par Mobile Money
                            </button>
                          ) : (
                            <div style={{border:"1px solid #E5E7EB",borderRadius:10,padding:10,background:"#FAFAFA"}}>
                              <div style={{fontSize:10,color:G.gray,fontWeight:700,marginBottom:6,letterSpacing:0.3}}>OPÉRATEUR</div>
                              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
                                {[{c:"ORANGE_SN_API_CASH_IN",l:"Orange Money"},{c:"WAVE_SN_API_CASH_IN",l:"Wave"},{c:"FREE_SN_WALLET_CASH_IN",l:"Free Money"},{c:"EXPRESSO_SN_WALLET_CASH_IN",l:"E-Money"}].map(o=>(
                                  <button key={o.c} onClick={()=>setIntechOp(o.c)}
                                    style={{flex:"1 0 45%",background:intechOp===o.c?p.color:"#FFF",color:intechOp===o.c?"#FFF":G.dark,border:`1px solid ${intechOp===o.c?p.color:"#D1D5DB"}`,borderRadius:8,padding:"7px 4px",fontSize:11,fontWeight:600,cursor:"pointer"}}>
                                    {o.l}
                                  </button>
                                ))}
                              </div>
                              <input type="tel" value={intechPhone} onChange={e=>setIntechPhone(e.target.value)} placeholder="Numéro à débiter (ex: 77 000 00 00)"
                                style={{width:"100%",boxSizing:"border-box",border:"1px solid #D1D5DB",borderRadius:8,padding:"8px 10px",fontSize:12,marginBottom:8,outline:"none",color:G.dark}}/>
                              <button disabled={intechPayState==="pending"} onClick={()=>startIntechPlanPayment(p.key, intechOp, intechPhone)}
                                style={{width:"100%",background:intechPayState==="pending"?"#9CA3AF":p.color,color:"#FFF",border:"none",borderRadius:8,padding:"10px 0",fontWeight:700,fontSize:12,cursor:intechPayState==="pending"?"default":"pointer"}}>
                                {intechPayState==="pending" ? "Paiement en cours… 📲" : intechPayState==="SUCCESS" ? "Payé ✓" : `Payer ${fmtMoney(p.priceNum, currency)} ${cur.short}`}
                              </button>
                              <button onClick={()=>{ setIntechPanel(null); setIntechPayState("idle"); }}
                                style={{width:"100%",background:"none",border:"none",color:G.gray,fontSize:11,marginTop:6,cursor:"pointer"}}>Annuler</button>
                            </div>
                          )}
                        </div>
                      )}
  ...
  <Trash2 size={13} style={{display:"inline",verticalAlign:"-2px"}}/> Supprimer la commande
                  </button>
                )}
    ...
    <div style={{marginBottom:8,display:"flex",justifyContent:"center"}}><X size={40} color={G.red}/></div>
                ...
                <div style={{display:"flex",gap:10}}>
                  ...
                  <button onClick={()=>{
                    ...
                    style={{flex:2,background:"#DC2626",color:"#fff",border:"none",borderRadius:14,padding:"15px 0",fontWeight:800,fontSize:16,cursor:"pointer",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:6}}><X size={18}/> Confirmer — Non livré</button>
                </div>
              </>
            )}
          </div>
